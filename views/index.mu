<!doctype html>
<meta charset="utf-8">
<meta name="keywords" content="Православное братство в честь святителя Спиридона Тримифунтского, братья, сестры, волонтеры,  милосердие, благотворительность, психоневрологический интернат, мероприятия, новости, Тарасики, Молодечно, д. Куль" />
<meta name="robots" content="index, follow">

<link rel="stylesheet" href="/css/base.css">
<link rel="stylesheet" href="/css/main.css">

<script src="/js/kernel.js"></script>
<script src="/js/main.js"></script>

<title>Братство в честь святителя Спиридона Тримифунтского</title>

<div class="modal-container">
  
</div>

<div class="main-container">

  <header class="global-header">
    <div class="icon"></div>
    <div class="name">
      <h1>Братство в честь святителя Спиридона Тримифунтского</h1>
    </div>
  </header>
  
  <div class="columns">








    <!-- NEWS -->

    <div class="main-content-col">


      <section class="news-list">

        <article class="news-view">
          <p><figure class="news-img"><img src="/1.png" width="254" alt=""></figure></p>
          <p><time class="news-date">10 минут назад</time><p>
          <p class="news-body">Видео «Десница Святителя Спиридона в Жировичах»</p>
        </article>

        <article class="news-view">
          <p><time class="news-date">врера в 23:30</time></p>
          <p class="news-body">Выражаем благодарность братьям и сестрам из Заславля, а также Сергию и Веронике (из Мачулищ) за помощь в организации паломнической поездки в храм Преображения Господня.</p>
        </article>

        <article class="news-view">
          <p><figure class="news-img"><img src="/2.png" width="254" alt=""></figure></p>
          <p><time class="news-date"> 23 МИНУТЫ НАЗАД</time></p>
          <p class="news-body">Видео «Святитель Спиридон Тримифунтский»</p>
        </article>

        <article class="news-view">
          <p><time class="news-date">12 МАЯ</time></p>
          <p class="news-body">Фотографии встречи и проводов мощей святителя Спиридона нашими братчиками.</p>
        </article>

        <article class="news-view">
          <p><figure class="news-img"><img src="/3.png" width="254" alt=""></figure></p>
          <p><time class="news-date">11 МАЯ</time></p>
          <p class="news-body">Наши подопечные столбцовского ПНИ (д. Куль) заняли в республиканском чемпионате по футболу среди первое место.</p>
          <p><a href="#" class="readmore">Читать</a></p>
        </article>

        <article class="news-view">
          <p><time class="news-date">10 МАЯ</time></p>
          <p class="news-body">Сегодня состоялась экскурсия на аэродроме «Боровая» проживающих ПНИ «Дражня».
Выражаем благодарность брату Юрию (Стефановичу) и брату Андрею (Минчику).</p>
        </article>


      </section> 
    </div>










    <!-- EVENTS -->
    
    <div class="aside-content-col">


      <section class="events-list">
      
        <section>

          <header>
            <h2><i class="icon-calendar"></i> Мероприятия</h2>
            <p class="small">Обновлено <em>десять минут назад</em></p>
          </header>
                
          <article class="event-view">
            <div class="event-short">
              <div class="event-state"></div>
              <div class="event-date">
                <div class="day">19</div>
                <div class="month">мая</div>
              </div>
              <div class="event-desc">
                Выезд проживающих ПНИ д. Тарасики на литургию и на Минское море
              </div>
            </div>
            <div class="event-more">
              <dl>
                <dt><i class="icon-time"></i> Место проведения</dt> <dd>Храм Преображения Господня, г. Заславь</dd>
                <dt><i class="icon-time"></i> Время проведения</dt> <dd>19 мая, воскресенье, в 12:00</dd>
                <dt><i class="icon-user"></i> Организатор</dt>      <dd>брат Сергий</dd>
                <dt><i class="icon-phone"></i> Телефон</dt>         <dd>8 (029) 312-12-23</dd>
              </dl>
            </div>
          </article>
                
          <article class="event-view">
            <div class="event-short">
              <div class="event-state new"></div>
              <div class="event-date">
                <div class="day">20</div>
                <div class="month">мая</div>
              </div>
              <div class="event-desc">
                Встреча с сестрами Свято-Димитриевского сестричества</a>
              </div>
            </div>
            <div class="event-more">
              <dl>
                <dt><i class="icon-time"></i> Место проведения</dt> <dd>Храм Преображения Господня, г. Заславь</dd>
                <dt><i class="icon-time"></i> Время проведения</dt> <dd>19 мая, воскресенье, в 12:00</dd>
                <dt><i class="icon-user"></i> Организатор</dt>      <dd>брат Сергий</dd>
                <dt><i class="icon-phone"></i> Телефон</dt>         <dd>8 (029) 312-12-23</dd>
              </dl>
            </div>
          </article>
                
          <article class="event-view new">
            <div class="event-short">
              <div class="event-state"></div>
              <div class="event-date">
                <div class="day">21</div>
                <div class="month">мая</div>
              </div>
              <div class="event-desc">
                Благотворительный концерт <a href="https://www.facebook.com/pages/Ambre-Hammond/112652052110017">Ambre Hammond</a>
              </div>
            </div>
            <div class="event-more">
              <p><b>Всех желающих поехать просьба брата Сергия предупредить заранее.</b></p>
              <dl>
                <dt><i class="icon-map-marker"></i> Место проведения</dt> <dd>Храм Преображения Господня, г. Заславь</dd>
                <dt><i class="icon-time"></i> Время проведения</dt> <dd>19 мая, воскресенье, в 12:00</dd>
                <dt><i class="icon-user"></i> Организатор</dt>      <dd>брат Сергий</dd>
                <dt><i class="icon-phone"></i> Телефон</dt>         <dd>8 (029) 312-12-23</dd>
              </dl>
            </div>
          </article>

        </section>

        <section class="archive">

          <header>
            <h2><i class="icon-calendar"></i> Архив событий</h2>
          </header>

          <article class="event-view">
            <div class="event-short">
              <div class="event-state"></div>
              <div class="event-date">
                <div class="day">21</div>
                <div class="month">мая</div>
              </div>
              <div class="event-desc">
                Благотворительный концерт <a href="https://www.facebook.com/pages/Ambre-Hammond/112652052110017">Ambre Hammond</a>
              </div>
            </div>
            <div class="event-more">
              <p><b>Всех желающих поехать просьба брата Сергия предупредить заранее.</b></p>
              <dl>
                <dt><i class="icon-map-marker"></i> Место проведения</dt> <dd>Храм Преображения Господня, г. Заславь</dd>
                <dt><i class="icon-time"></i> Время проведения</dt> <dd>19 мая, воскресенье, в 12:00</dd>
                <dt><i class="icon-user"></i> Организатор</dt>      <dd>брат Сергий</dd>
                <dt><i class="icon-phone"></i> Телефон</dt>         <dd>8 (029) 312-12-23</dd>
              </dl>
            </div>
          </article>

        </section>

      </section>
    </div>

  </div>
</div>